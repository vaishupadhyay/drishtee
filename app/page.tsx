    ...
    <div className="app-frame min-h-screen bg-surface text-navy">
      <header className="border-b border-assist-line bg-surface/95">
        <div className="mx-auto flex min-h-20 w-[min(1180px,calc(100%-32px))] items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2"><MobileViewToggle /><Link href="/help#language" className="hidden rounded-full border border-assist-line bg-white px-3 py-2 text-xs font-bold text-[#45566D] sm:block">English ▾</Link><Link href="/auth/login" className="hidden rounded-full border border-assist-line bg-white px-3 py-2 text-xs font-bold text-[#45566D] md:block">Login / Sign up</Link><Link href="/assist" className="rounded-full bg-navy px-5 py-3 text-sm font-extrabold !text-white shadow-[0_8px_18px_rgb(11_31_61_/_20%)] transition hover:bg-navy-2 active:scale-[.98]">Report an issue</Link></div>
        </div>
      </header>
        ...
        <section className="overflow-hidden py-14 md:py-[74px]">
          <div className="mx-auto grid w-[min(1180px,calc(100%-32px))] items-center gap-12 lg:grid-cols-[1.06fr_.94fr] lg:gap-20">
            <div>
              ...
              <div id="report-options" className="mt-7 scroll-mt-24 grid gap-3 sm:grid-cols-2">
                <Link href="/scan" className="relative min-h-[178px] rounded-[20px] border border-[#CFECD9] bg-mint p-5 transition hover:-translate-y-1 hover:shadow-[0_14px_32px_rgb(23_52_92_/_10%)]"><span className="absolute right-4 top-4 rounded-full bg-white px-2 py-1 text-[10px] font-extrabold text-mint-strong">Recommended</span><span className="grid h-10 w-10 place-items-center rounded-xl bg-mint-strong text-white" aria-hidden="true"><svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M8 3H5a2 2 0 0 0-2 2v3M16 3h3a2 2 0 0 1 2 2v3M8 21H5a2 2 0 0 1-2-2v-3M16 21h3a2 2 0 0 0 2-2v-3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="12" r="3" stroke="currentColor" strokeWidth="2"/></svg></span><h2 className="mt-4 text-[19px] font-extrabold tracking-[-.025em]">Scan and Report</h2><p className="mt-1 text-xs leading-5 text-[#4E6070]">Show a sign, notice, receipt or counter, then tell us what happened.</p><span className="mt-4 inline-flex rounded-full bg-mint-strong px-2.5 py-1.5 text-[11px] font-extrabold text-white">Start scanning →</span></Link>
